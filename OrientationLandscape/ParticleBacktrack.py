...
def read_relion_star(star):
    """
    Function to read the relion star file and parse the metadata.
    """
            ...
            for num, line in enumerate(starfile):
                ...
                if re.match(r'data_', sline):
                    if re.match(r'data_optics', sline):
                        ...
                        raise ValueError(f"found data label not either _optics or _particles ... line {num}")
                    continue
    ...
    return header, pd.DataFrame(particles_metadata_table), pd.DataFrame(
        optics_metadata_table), particles_metadata_index, optics_metadata_index


def backtrack_particles(extracted_file, star_file, output_file):
    """
    Function to backtrack the particles and write the result into the output file.
    """
    import pandas as pd
    import argparse
    import re
    ...
    def read_relion_star(star):
        """
        Reads a RELION .star file and extracts metadata.

        Returns:
        - header (str): The header lines from the star file.
        - particles_metadata_df (pd.DataFrame): DataFrame containing particle metadata.
        - optics_metadata_df (pd.DataFrame): DataFrame containing optics metadata.
        - particles_metadata_index (dict): Mapping of column names to their indices for particles metadata.
        - optics_metadata_index (dict): Mapping of column names to their indices for optics metadata.
        """
        optics_metadata_index = {}
        optics_metadata_index_found = 0
        optics_metadata_table = []
    ...
    def backtrack_particles(extracted_file, star_file, output_file):
        """
        Backtrack the particles specified by their ID in the extracted file and write them
        from the original star file to the output file.

        Parameters:
        - extracted_file (str): Path to the CSV file with extracted particles (must have 'ID' column).
        - star_file (str): Path to the original RELION star file.
        - output_file (str): Path to the output star file.
        """
