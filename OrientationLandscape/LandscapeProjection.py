...
def compute_azimuth_elevation(rotvecs):
    # Compute the norm of each rotation vector
...
def main():
    ...
    if args.rotvec_limits:
        ...
        # Optionally, set default limits for rotation vectors based on data range
        # Here, we'll use the data's min and max values
        for col in rotvec_columns:
            min_limit = data[col].min()
            max_limit = data[col].max()
            rotvec_limits[col] = (min_limit, max_limit)
